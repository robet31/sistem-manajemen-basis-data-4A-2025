...
`skala_umkm``produk_umkm``pemilik_umkm``kategori_umkm``kabupaten_kota`
...
SELECT u.nama_usaha, p.nama_lengkap AS nama_pemilik, k.nama_kategori, s.nama_skala, kk.nama_kabupaten_kota, u.tahun_berdiri
FROM umkm u 
...
AS SELECT u.nama_usaha, p.nama_lengkap AS nama_pemilik
FROM umkm u
JOIN pemilik_umkm p ON u.id_pemilik = p.id_pemilik
JOIN skala_umkm s ON u.id_skala = s.id_skala
